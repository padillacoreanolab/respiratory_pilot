...
def trodes_extract(trodes_directory, data, mode="-raw"):
    """
    Automates Trodes extraction using trodesexport and places output folders
    inside each folder containing a .rec file using -outputdirectory.
    
    Args:
        trodes_directory: str, path to Trodes installation directory
        data: str, path to folder containing subfolders with .rec files
        mode: str, export mode(s) like '-raw -dio -analogio'
    """
    print(f"\n[INFO] Starting Trodes extraction with mode(s): {mode}")
    print(f"[INFO] Trodes directory: {trodes_directory}")
    print(f"[INFO] Data folder: {data}")

    data_path = os.path.join(data, "*")
    folders = glob.glob(data_path)
    print(f"[INFO] Found {len(folders)} subdirectories to search for .rec files.")

    files = [glob.glob(os.path.join(folder, "*merged.rec")) for folder in folders]

    commands = []
    for i, folder in enumerate(folders):
        print(f"\n[INFO] Checking folder: {folder}")
        if files[i]:
            print(f"[INFO] Found {len(files[i])} merged.rec file(s).")
            for file in files[i]:
                output_dir = folder
                command = (
                    f'trodesexport {mode} -rec "{file}" '
                    f'-outputdirectory "{output_dir}"'
                )
                print(f"[DEBUG] Prepared command: {command}")
                commands.append((command, output_dir))
        else:
            print(f"[WARN] No merged.rec file found in {folder}")

    if not commands:
        print("\n[ERROR] No commands generated. Make sure .rec files exist in subdirectories.")
        return

    print(f"\n[INFO] Executing {len(commands)} extraction command(s)...\n")
    for i, (command, output_dir) in enumerate(commands):
        print(f"[RUNNING] ({i+1}/{len(commands)}) Extracting to: {output_dir}")
        process = subprocess.Popen(command, shell=True, cwd=trodes_directory)
        process.wait()
        if process.returncode == 0:
            print(f"[SUCCESS] Extraction complete for: {output_dir}")
        else:
            print(f"[ERROR] Extraction failed for: {output_dir} (exit code {process.returncode})")

    print("\n[INFO] Trodes extraction process finished.")
